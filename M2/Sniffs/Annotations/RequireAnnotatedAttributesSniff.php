...
class M2_Sniffs_Annotations_RequireAnnotatedAttributesSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
    /**
    ...
    protected function extractVarDocBlock($tokens, $commentToken, $stackPtr)
    {
        ...
        if ($commentEnd !== false && $tokens[$commentEnd]['code'] === T_COMMENT) {
            $this->helper->addMessage($stackPtr, M2_Sniffs_Annotations_Helper::WRONG_STYLE, ['variable']);
            $break = true;
        } elseif ($commentEnd === false || $tokens[$commentEnd]['code'] !== T_DOC_COMMENT) {
            $this->helper->addMessage($stackPtr, M2_Sniffs_Annotations_Helper::MISSING, ['variable']);
            $break = true;
        } else {
    ...
    protected function checkForDescription($comment, $commentStart)
    {
        ...
        $newlineCount = 0;
        if (trim($short) === '') {
            $this->helper->addMessage($commentStart, M2_Sniffs_Annotations_Helper::MISSING_SHORT, ['variable']);
            $newlineCount = 1;
        } else {
            // No extra newline before short description.
            $newlineSpan = strspn($short, $this->helper->getEolChar());
            if ($short !== '' && $newlineSpan > 0) {
                $this->helper->addMessage($commentStart + 1, M2_Sniffs_Annotations_Helper::SPACING_BEFORE_SHORT, ['variable']);
            }
            ...
            if (empty($long) === false) {
                ...
                if ($newlineBetween !== 2) {
                    $this->helper->addMessage(
                        ...
                        M2_Sniffs_Annotations_Helper::SPACING_BETWEEN,
                        ['variable']
                    );
                }
                ...
                if (preg_match('|\p{Lu}|u', $testLong[0]) === 0) {
                    $this->helper->addMessage(
                        ...
                        M2_Sniffs_Annotations_Helper::LONG_NOT_CAPITAL,
                        ['Variable']
                    );
                }
            }
            ...
            if (substr_count($testShort, $this->helper->getEolChar()) !== 0) {
                $this->helper->addMessage($commentStart + 1, M2_Sniffs_Annotations_Helper::SHORT_SINGLE_LINE, ['Variable']);
            }

            if (preg_match('|\p{Lu}|u', $testShort[0]) === 0) {
                $this->helper->addMessage($commentStart + 1, M2_Sniffs_Annotations_Helper::SHORT_NOT_CAPITAL, ['Variable']);
            }

            if ($lastChar !== '.') {
                $this->helper->addMessage($commentStart + 1, M2_Sniffs_Annotations_Helper::SHORT_FULL_STOP, ['Variable']);
            }
        }
        ...
        if (count($tags) > 1) {
            ...
            if ($newlineSpan !== 2) {
                ...
                $this->helper->addMessage(
                    $commentStart + $newlineCount,
                    M2_Sniffs_Annotations_Helper::SPACING_BEFORE_TAGS,
                    ['variable']
                );
